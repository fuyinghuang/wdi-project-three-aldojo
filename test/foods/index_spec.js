    ...
    api.get('/api/foods')
      .end((err, res) => {
        res.body.forEach(food => {
          const dataItem = foodData.find(item => item.name === food.name);
          expect(food.name).to.eq(dataItem.name);
          expect(food.description).to.eq(dataItem.description);
          expect(food.image).to.eq(dataItem.image);
          expect(food.country).to.eq(dataItem.country);
          // expect(food.comments[0].commentAuthor).to.eq(dataItem.comments[0].commentAuthor);
          // expect(food.comments[0].text).to.eq(dataItem.comments[0].text);
        });
        done();
      });
  });
